...
export const SUGGESTION_GROUPS: { category: string; prompts: string[] }[] = [
  ...
  },
  {
    ...
    prompts: [
      ...
      'Break down our documents by source system',
...
/**
 * SOURCE-SYSTEM question — "which notices come from outside the core?", "what's
 * built in house?", "where does our data come from?".
 *
 * Every data-driven document is composed from a file, and only some of those
 * files come off the FI's core processor. The rest arrive from ancillary
 * platforms or are assembled by the FI. Each non-core source is a separate
 * feed to map and maintain, which is why this is the first question asked in a
 * core conversion or a custom-programming scope.
 */
function answerSource(q: string, clientId: string | null): AIResponse | null {
  const mentionsCore = /\bcores?\b|core system|core processor|symitar|episys|keystone|corelation/i.test(q)
  const mentionsSource =
    /source system|which system|what system|where (do|does|are).{0,30}(come|coming) from|originat|generated by|produced by|feed|data file/i.test(q)
  // unambiguous in this app — enough on their own to mean the source question
  const strongOutside =
    /outside (the |our )?core|not (from|on|in) (the )?core|non-?core|off-?core|in.?house|third.?part|ancillary/i.test(q)
  // too generic to route on without a document subject alongside them
  const weakOutside = /outside|other than|besides|apart from|external|vendor|elsewhere/i.test(q)
  const mentionsOutside = strongOutside || weakOutside
  if (!mentionsCore && !mentionsSource && !mentionsOutside) return null
  if (
    !mentionsCore &&
    !mentionsSource &&
    !strongOutside &&
    !/\b(notice|statement|tax|document|design)/i.test(q)
  )
    return null

  const cf = categoryFilter(q)
  const docs = clientDocs(clientId).filter(
    (d) => d.source && (cf ? d.category === cf.cat : true),
  )
  if (!docs.length) return null

  const scope = cf ? cf.label : 'data-driven documents'
  const coreDocs = docs.filter((d) => d.source!.core)
  const offDocs = docs.filter((d) => !d.source!.core)
  const coreName =
    clientId
      ? (CLIENTS.find((c) => c.id === clientId)?.coreSystem ?? 'the core')
      : 'the core'
  const owner = clientId ? (CLIENTS.find((c) => c.id === clientId)?.shortName ?? '') + "'s " : ''

  // Which side of the split is being asked about. "outside/not/non-core" wins
  // over a bare "from the core", since the negative phrasing contains both.
  const wantsOff = mentionsOutside
  const wantsCore = !wantsOff && /\bfrom (the )?core|on (the )?core|core[- ]generated|come.{0,12}core/i.test(q)

  /** Group a set of documents by their source system, largest group first. */
  const groupBySystem = (list: Doc[]) => {
  ...
  const TABLE_CAP = 25
  const moreNote = (list: Doc[]) =>
    list.length > TABLE_CAP
      ? `\n\nThe table below lists the first ${TABLE_CAP}; ${list.length - TABLE_CAP} more are in the repository.`
      : ''
  const detailTable = (list: Doc[]) => ({
    headers: ['Document', 'Source system', 'Current version', 'Last updated'],
    rows: list.slice(0, TABLE_CAP).map((d) => {
      const l = d.versions[d.versions.length - 1]
      return [d.name, d.source!.system, `v${l.v}`, fmtDate(l.date)] as (string | number)[]
    }),
  })
  ...
  if (wantsOff) {
    if (!offDocs.length) {
      return {
        text: `Every one of ${owner}**${docs.length} ${scope}** is produced by **${coreName}**. Nothing sits outside the core.`,
        cites: [],
        suggestions: SUGGESTIONS,
      }
    }
    const groups = groupBySystem(offDocs)
    const bullets = groups
      .map(
        ([sys, list]) =>
          `• **${sys}** — ${list.length} ${list.length === 1 ? scope.replace(/s$/, '') : scope}\n   ${list[0].source!.note}`,
      )
      .join('\n')
    return {
      text:
        `**${offDocs.length} of ${owner}${docs.length} ${scope}** are produced **outside ${coreName}**, ` +
        `across ${groups.length} ${groups.length === 1 ? 'system' : 'different systems'}. ` +
        `The other ${coreDocs.length} come straight off the core.\n\n${bullets}\n\n` +
        `Each non-core source is its own data file to map, test and maintain — worth scoping separately in a conversion or a redesign.` +
        moreNote(offDocs),
      cites: offDocs.slice(0, 12).map((d) => ({
        docId: d.id,
        name: d.name,
        v: d.versions[d.versions.length - 1].v,
      })),
      table: detailTable(offDocs),
      suggestions: SUGGESTIONS,
    }
  }
  ...
  if (wantsCore) {
    return {
      text:
        `**${coreDocs.length} of ${owner}${docs.length} ${scope}** are produced by **${coreName}** — standard core output, ` +
        `delivered in the nightly extract. The other ${offDocs.length} come from ${groupBySystem(offDocs).length} other systems.` +
        moreNote(coreDocs),
      cites: coreDocs.slice(0, 12).map((d) => ({
        docId: d.id,
        name: d.name,
        v: d.versions[d.versions.length - 1].v,
      })),
      table: detailTable(coreDocs),
      suggestions: SUGGESTIONS,
    }
  }

  // ---- full breakdown by system ----
  const groups = groupBySystem(docs)
  return {
    text:
      `${owner || 'Your '}**${docs.length} ${scope}** are composed from ` +
      `${groups.length} different source systems. **${coreName}** produces ${coreDocs.length}; ` +
      `the remaining ${offDocs.length} come from elsewhere:\n\n` +
      groups
        .map(([sys, list]) => `• **${sys}**${list[0].source!.core ? ' *(core)*' : ''} — ${list.length}`)
        .join('\n'),
    cites: [],
    table: {
      headers: ['Source system', 'On the core?', 'Documents'],
      rows: groups.map(([sys, list]) => [sys, list[0].source!.core ? 'Yes' : 'No', list.length]),
    },
    suggestions: SUGGESTIONS,
  }
}
...
export function askAI(query: string, clientId: string | null): AIResponse {
  ...
  const handlers = [
    ...
    // source-system questions run before inventory so "how many notices come
    // from the core" isn't swallowed by the generic "how many" counter.
    () => answerSource(q, clientId),
